...
class Trainer:
    def __init__(self, config):
        ...
        self.TEST_INITIAL_PREDICTOR_WEIGHT_PATH = (
            config.TEST_INITIAL_PREDICTOR_WEIGHT_PATH
        )
        ...
        if self.mode == 1 and self.continue_training == "True":
            ...
            self.network.init_predictor.load_state_dict(
                torch.load(self.pretrained_path_init_predictor)
            )
        ...
        if self.mode == 1:
            dataset_train = DocData(
                self.path_train_img, self.path_train_gt, config.IMAGE_SIZE, self.mode
            )
            ...
            dataset_test = DocData(
                config.TEST_PATH_IMG, config.TEST_PATH_GT, config.IMAGE_SIZE, self.mode
            )
    ...
    def test(self):
        ...
        with torch.no_grad():
            self.network.init_predictor.load_state_dict(
                torch.load(self.TEST_INITIAL_PREDICTOR_WEIGHT_PATH)
            )
            self.network.denoiser.load_state_dict(
                torch.load(self.TEST_DENOISER_WEIGHT_PATH)
            )
            ...
            for img, gt, name in tq:
                ...
                save_image(
                    img_save,
                    os.path.join(self.test_img_save_path, f"{name[0]}.png"),
                    nrow=4,
                )
    ...
    def train(self):
        ...
        while iteration < self.iteration_max:
            ...
            for img, gt, name in tq:
                ...
                init_predict, noise_pred, noisy_image, noise_ref = self.network(
                    gt.to(self.device), img.to(self.device), t, self.diffusion
                )
                if self.pre_ori == "True":
                    if self.high_low_freq == "True":
                        residual_high = self.high_filter(
                            gt.to(self.device) - init_predict
                        )
                        ddpm_loss = 2 * self.loss(
                            self.high_filter(noise_pred), residual_high
                        ) + self.loss(noise_pred, gt.to(self.device) - init_predict)
                    else:
                        ddpm_loss = self.loss(
                            noise_pred, gt.to(self.device) - init_predict
                        )
                else:
                    ddpm_loss = self.loss(noise_pred, noise_ref.to(self.device))
                if self.high_low_freq == "True":
                    low_high_loss = self.loss(init_predict, gt.to(self.device))
                    low_freq_loss = self.loss(
                        init_predict - self.high_filter(init_predict),
                ...
                if self.high_low_freq == "True":
                    tq.set_postfix(
                        loss=loss.item(),
                        high_freq_ddpm_loss=ddpm_loss.item(),
                        low_freq_pixel_loss=low_freq_loss.item(),
                        pixel_loss=low_high_loss.item(),
                    )
                else:
                    tq.set_postfix(
                        loss=loss.item(),
                ...
                if iteration % 500 == 0:
                        ...
                        if self.high_low_freq == "True":
                            img_save = torch.cat(
                                [
                                    ...
                                    init_predict.cpu(),
                                    noise_pred.cpu()
                                    + self.high_filter(init_predict).cpu(),
                                    noise_pred.cpu() + init_predict.cpu(),
                                ],
                                dim=3,
                            )
                        else:
                    ...
                    save_image(
                        img_save,
                        os.path.join(save_img_path, f"{iteration}.png"),
                        nrow=4,
                    )
                iteration += 1
                ...
                if iteration % self.save_model_every == 0:
                    ...
                    torch.save(
                        self.network.init_predictor.state_dict(),
                        os.path.join(
                            self.weight_save_path, f"model_init_{iteration}.pth"
                        ),
                    )
                    torch.save(
                    ...
                    if self.EMA_or_not == "True":
                        torch.save(
                            self.ema_model.init_predictor.state_dict(),
                            os.path.join(
                                self.weight_save_path, f"model_init_ema_{iteration}.pth"
                            ),
                        )
                        torch.save(
                            self.ema_model.denoiser.state_dict(),
                            os.path.join(
                                self.weight_save_path,
                                f"model_denoiser_ema_{iteration}.pth",
                            ),
                        )
